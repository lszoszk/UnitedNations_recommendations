    ...
    if (state.searchPage === 1 && !(r.records || []).length) {
      ...
      $('#seList').innerHTML = `
        <div class="empty-state">
          ...
          <div class="es-sub">${hasFilters ? 'Try removing one or two filters, broadening the year range, or using a different keyword.' : 'Hmm, the dataset should have 272,502 records — something went wrong.'}</div>
          ${kwActive ? `
            <div class="se-warn-hint">
