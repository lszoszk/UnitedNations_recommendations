...
function renderMethodology() {
  $('#view-methodology').innerHTML = `
    <div class="me">
      ...
      <h2>Data source</h2>
      <p>All records originate from the OHCHR <strong>Universal Human Rights Index</strong> (<a href="https://uhri.ohchr.org/" target="_blank" rel="noopener">uhri.ohchr.org</a>). By default this dashboard shows data processed by our transparent <strong>five-stage content-cleaning pipeline</strong> (detailed below). You can switch to the <strong>raw upstream text</strong> using the <code>DATA · cleaned ▾</code> pill in the header — raw mode re-includes the 11 artefact rows and runs search against the original HTML-stripped text. Country, body and theme metadata remain pipeline-canonical in either mode. Share a raw-mode view by appending <code>?dataset=raw</code> to the URL.</p>
      ...
      <p style="color:var(--dim);font-size:12px;margin-top:-6px"><strong>If you need the full 267,548</strong> — flip the header toggle to <code>DATA · raw</code>. The 11 artefacts come back along with the original HTML-ridden text for every other record. Most researchers prefer cleaned; the raw toggle exists for reproducibility + for anyone auditing the pipeline against the upstream export.</p>

      <h2>Country coverage</h2>
      <p>The hex map shows <strong>198 state parties</strong>, derived from UN M49 member states + observer states that appear (or plausibly could appear) in UHRI records. <strong>197</strong> of them have at least one Treaty Body / UPR / Special Procedure recommendation or observation attached. <strong>Bermuda</strong> alone has zero — it's a UK Overseas Territory, not a UN Member State, so OHCHR doesn't issue recommendations on it. Its hex stays in the layout as a geographic reference and always renders grey.</p>
      <p style="color:var(--dim);font-size:12px;margin-top:-6px">Two entries the upstream API carries that the hex map doesn't: the <em>European Union</em> as a bloc (233 records — not a sovereign state, so no hex) and <em>Kosovo*</em> (75 records — not in UN M49 due to its contested status; on the roadmap as a special-case hex).</p>
